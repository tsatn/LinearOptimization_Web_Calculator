<template>
  <div class="container flex justify-center items-center min-h-screen p-6">
    <div class="bg-[#F7F5EB] p-8 rounded-lg shadow-lg max-w-lg w-full">
      <h2 class="text-3xl font-bold text-center text-[#374259] mb-6 font-playfair">Convex Optimization Calculator</h2>
      <form @submit.prevent="submitForm" class="space-y-6" style="position: relative; flex-direction: column; align-items: center; justify-content: center; width: 100%;">
  ...
  setup() {
    ...
    const submitForm = async () => {
      try {
        loading.value = true;
        error.value = null;
        const response = await axios.post('/api/convexOptimization', {
